...
namespace Ueg\Crm\Block\Adminhtml\Customer\Edit\Tab\Coin;
...
//class Ueg_Crm_Block_Adminhtml_Customer_Edit_Tab_Coin_Grid extends Mage_Adminhtml_Block_Widget_Grid
    ...
    \Magento\Backend\Block\Template\Context $context, \Magento\Backend\Helper\Data $backendHelper, \Ueg\Crm\Model\CoinFactory $CoinFactory, \Ueg\Crm\Model\Coinoptions $coinOptions, \Ueg\Crm\Model\Customer\Attribute\Source\CustomCustomerAssignedToOptions $AssignOption, array $data = []
    ) {
    ...
    public function callback_datefield($value, $row, $column, $isExport) {
        ...
        $id = $row->getId();
        $fieldName = $column->getIndex();
        $html .= '<div class="range-line date"><input id="request-date-' . $id . '" type="text" name="coin[' . $id . '][' . $fieldName . ']" class="text-box" value="' . date('m/j/y', strtotime($value)) . '"/></div>';
